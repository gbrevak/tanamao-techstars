    ...
    <div className="glass-surface rounded-card p-4">
      ...
      <div className="grid grid-cols-2 gap-3 mt-3">
        <div className="flex flex-col bg-muted/50 rounded-button p-2">
          ...
          <span className="text-xs sm:text-sm font-bold text-money truncate">
            {valoresVisiveis ? `R$ ${totaisEntradas.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}` : '•••••'}
          </span>
          <span className="text-[9px] sm:text-[10px] text-muted-foreground">{valoresVisiveis ? `${pctEntradas.toFixed(0)}% do total` : '•••'}</span>
        </div>
        <div className="flex flex-col bg-muted/50 rounded-button p-2">
          <span className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
            <span className="w-2 h-2 rounded-full bg-expense" /> Saiu
          </span>
          <span className="text-xs sm:text-sm font-bold text-expense truncate">
            {valoresVisiveis ? `R$ ${totaisSaidas.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}` : '•••••'}
          </span>
          <span className="text-[9px] sm:text-[10px] text-muted-foreground">{valoresVisiveis ? `${pctSaidas.toFixed(0)}% do total` : '•••'}</span>
        </div>
      </div>
    </div>
  );
